...
# KNOWLEDGE stores both general advice/tips ('advice') and executable commands ('policy').
...
KNOWLEDGE: Dict[str, Any] = {
    ...
    "policy": {
        ...
        "verify_install": "Verify installation and check version: `conda info`",
        "update_conda": "Update Conda in the base environment: `conda update --name base conda`",
        "clean_all": "Remove all unused files: `conda clean --all`",
        
        # Environment Management Commands
        "list_envs": "List all environments and their locations: `conda info --envs`",
        "create_env_base": "Create a new environment: `conda create --name ENVNAME`",
        "activate_env": "Activate an environment: `conda activate ENVNAME`",
        "install_pkg_env": "Install packages in a specific environment: `conda install --name ENVNAME PKGNAME1 PKGNAME2`",
        "delete_env": "Delete an environment by name: `conda remove --name ENVNAME --all`",
        
        # Export/Import Commands
        "export_history_yml": "Export cross-platform compatible settings: `conda export --from-history>ENV.yml`",
        "export_explicit_txt": "Export explicit settings (platform/channel specific): `conda list --explicit>ENV.txt`",
        "import_yml": "Create an environment from a .yml file: `conda env create --name ENVNAME --file ENV.yml`",
        
        # Revision Commands
        "list_revisions": "List revisions made to an environment: `conda list --name ENVNAME --revisions`",
    }
}
...
RULES: List[Dict[str, Any]] = [
    ...
    {"id": "conda_new_project_sequence",
     ...
     "conclusion": KNOWLEDGE["advice"]["new_project_start"] +
                   " To start, execute the policy: " + 
                   KNOWLEDGE["policy"]["create_env_base"]},
    ...
    {"id": "conda_install_requires_activation",
     ...
     "conclusion": KNOWLEDGE["advice"]["activation_sequence"] +
                   " You must activate the environment first. Use policy: " +
                   KNOWLEDGE["policy"]["activate_env"]},
    ...
    {"id": "conda_install_specific_env",
     ...
     "conclusion": KNOWLEDGE["advice"]["environment_scoping"] +
                   " Use the policy: " +
                   KNOWLEDGE["policy"]["install_pkg_env"]},
    ...
    {"id": "conda_export_portable",
     ...
     "conclusion": KNOWLEDGE["advice"]["export_cross_platform"] +
                   " " + KNOWLEDGE["advice"]["export_naming_tip"] +
                   " Policy: " + KNOWLEDGE["policy"]["export_history_yml"]},

    # R005: Export for Maximum Detail (Channel specific)
    {"id": "conda_export_detailed",
     "conditions": [{"attribute": "export_env", "op": "eq", "value": True},
                    {"attribute": "full_detail", "op": "eq", "value": True}],
     "conclusion": KNOWLEDGE["advice"]["detailed_export"] +
                   " Policy: " + KNOWLEDGE["policy"]["export_explicit_txt"]},
    ...
    {"id": "conda_dependency_error",
     ...
     "conclusion": "You may have a dependency conflict. Remember that " +
                   ...
                   " If resolution fails, try listing revisions using policy: " +
                   KNOWLEDGE["policy"]["list_revisions"]},
    ...
    {"id": "conda_system_cleanup",
     ...
     "conclusion": "To free disk space by clearing cache and unused packages, use the advice: " +
                   ...
                   " Policy: " + KNOWLEDGE["policy"]["clean_all"]}
]
...
class NLPProcessor:
    ...
    def process_query(self, text: str):
        ...
        if any(w in t for w in ["export", "save environment", "backup environment"]):
...
def present_answer(structured, conclusions, advice):
    ...
    if conclusions:
        console.print("\n[bold green]Rules Applied:[/bold green]")
        
        # Create table for rules
        rules_table = Table(show_header=True, header_style="bold magenta", box=box.ROUNDED)
        rules_table.add_column("Rule ID", style="yellow")
        rules_table.add_column("Conclusion", style="white")
        
        for c in conclusions:
            # Render markdown formatting in conclusions
            conclusion_text = Text.from_markup(c['conclusion'])
            rules_table.add_row(c['rule_id'], conclusion_text)
...
def handle_user_input():
    ...
    print("Conda Support Knowledge-Based System (KBS)")
    print("Type your Conda command question. Type 'help' for examples. Type 'quit' to exit.")
